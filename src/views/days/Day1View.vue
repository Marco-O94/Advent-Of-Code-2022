...
function getResult() {
    const calories = data.meals.split('\n');
    let calsList: Array<number> = []; 
    let calorySum = 0;   
    calories.forEach((cal) => {
        if(cal != '') {
            calorySum += parseInt(cal);
        }else{calsList.push(calorySum), calorySum = 0;}

    });
    let MaxCal: number = Math.max(...calsList);

    console.log(MaxCal);
    
    

    // Creare a multidimensional array with a string with \n as separator
...
<template>
    <div class="container p-3 bg-gray-50 rounded dark:bg-gray-800 dark:border-gray-700">
